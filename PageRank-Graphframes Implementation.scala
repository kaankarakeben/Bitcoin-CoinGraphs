...
// Selecting pubkeys from final data to construct the vertices of gprahs
val wallets = sqlContext.sql("select distinct src as pubkey from edges union all select distinct dst as pubkey from edges")
wallets.registerTempTable("wallets")
val vertices = sqlContext.sql("select distinct pubkey as id from wallets")

// Constructing GraphFrames Graph
val g = GraphFrame(vertices, edges)

// Running pageRank algorithm
val results = g.pageRank.tol(0.03).run()
